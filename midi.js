...
var casecur = 0;
 ...
 var use;
//converts this to midi data

function toMidi(array){

    var count = 0;
    var song = 0;
    noteEvents = []
    duration = 128/bpmdivision
    console.log("reached")
    ...
    for(var i = 0; i<array.length;i++){
        ...
        if ((array[i][0]+array[i][1])=="E#"){
            array[i]="F"+array[i][2]
        }
            if ((array[i][0]+array[i][1])=="B#"){
            array[i]="C"+array[i][2]
        }
// && inRange(lastvols[i],firstvols[i+1])
     note2=array[i] 

       if (note2!="NaN" && skip==true && array[i]==array[i+1]  && i<array.length && inRange(lastvols[i],firstvols[i+1]))
        {
            duration = duration+(128/bpmdivision)
             //Array.prototype.push.apply(noteEvents, MidiEvent.createNote(note2, duration, count));
            casecur = 1;
            
        }

//its replacing store note?
//fix this
        else if (skip==false)
        {
            casecur = 2;
            if (note2!="NaN"){
              ...
              if (array[i]==array[i+1] && inRange(lastvols[i],firstvols[i+1]))
                 {
                    storenote = note2;
                      casecur = 5;
                    skip = true;
        ...
        Array.prototype.push.apply(noteEvents, MidiEvent.createNote(storenote, duration, count));
        skip=false;
        duration = 128/bpmdivision;
        count = 0;
        //count = count + 128/bpmdivision
...
function midiDownload(){
    if (whichmidi="midinotes"){
        songs[0].save();
    }
    else{
        songs[1].save();
...
var DEFAULT_DURATION = 128/bpmdivision;
...
var noteTable = { "G9": 0x7F, "F#9": 0x7E, "F9": 0x7D, "E9": 0x7C, "D#9": 0x7B,
"D9": 0x7A, "C#9": 0x79, "C9": 0x78, "B8": 0x77, "A#8": 0x76, "A8": 0x75, "G#8": 0x74,
"G8": 0x73, "F#8": 0x72, "F8": 0x71, "E8": 0x70, "D#8": 0x6F, "D8": 0x6E, "C#8": 0x6D,
"C8": 0x6C, "B7": 0x6B, "A#7": 0x6A, "A7": 0x69, "G#7": 0x68, "G7": 0x67, "F#7": 0x66,
"F7": 0x65, "E7": 0x64, "D#7": 0x63, "D7": 0x62, "C#7": 0x61, "C7": 0x60, "B6": 0x5F,
"A#6": 0x5E, "A6": 0x5D, "G#6": 0x5C, "G6": 0x5B, "F#6": 0x5A, "F6": 0x59, "E6": 0x58,
"D#6": 0x57, "D6": 0x56, "C#6": 0x55, "C6": 0x54, "B5": 0x53, "A#5": 0x52, "A5": 0x51,
"G#5": 0x50, "G5": 0x4F, "F#5": 0x4E, "F5": 0x4D, "E5": 0x4C, "D#5": 0x4B, "D5": 0x4A,
"C#5": 0x49, "C5": 0x48, "B4": 0x47, "A#4": 0x46, "A4": 0x45, "G#4": 0x44, "G4": 0x43,
"F#4": 0x42, "F4": 0x41, "E4": 0x40, "D#4": 0x3F, "D4": 0x3E, "C#4": 0x3D, "C4": 0x3C,
"B3": 0x3B, "A#3": 0x3A, "A3": 0x39, "G#3": 0x38, "G3": 0x37, "F#3": 0x36, "F3": 0x35,
"E3": 0x34, "D#3": 0x33, "D3": 0x32, "C#3": 0x31, "C3": 0x30, "B2": 0x2F, "A#2": 0x2E,
"A2": 0x2D, "G#2": 0x2C, "G2": 0x2B, "F#2": 0x2A, "F2": 0x29, "E2": 0x28, "D#2": 0x27,
"D2": 0x26, "C#2": 0x25, "C2": 0x24, "B1": 0x23, "A#1": 0x22, "A1": 0x21, "G#1": 0x20,
"G1": 0x1F, "F#1": 0x1E, "F1": 0x1D, "E1": 0x1C, "D#1": 0x1B, "D1": 0x1A, "C#1": 0x19,
"C1": 0x18, "B0": 0x17, "A#0": 0x16, "A0": 0x15, "G#0": 0x14, "G0": 0x13, "F#0": 0x12,
"F0": 0x11, "E0": 0x10, "D#0": 0x0F, "D0": 0x0E, "C#0": 0x0D, "C0": 0x0C };
    ...
    if (config) {
        ...
        return {
            ...
            make: function() {
                ...
                use.setAttribute("src", "data:audio/midi;base64," + this.b64);
...
MidiEvent.createNote = function(note, duration, when, sustained) {
    ...
    var events = [];
    var noteused = MidiEvent.noteOn(note)
    noteused.setTime(when)
    events.push(noteused);
    ...
    if (!sustained) {
        ...
        events.push(MidiEvent.noteOff(note, duration || DEFAULT_DURATION));
    }
